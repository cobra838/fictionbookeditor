	...
	for (int j = 0; j < *length; j++) {
		if (text[j] == '\\')
		{
			if (text[j + 1] >= '1' && text[j + 1] <= '9') {
				...
				if (search.pat[patNum] && len != 0)	// Will be null if try for a match that did not occur
...
long BuiltinRegex::FindText(Document *doc, int minPos, int maxPos, const char *s,
	...
	for (int line = lineRangeStart; line != lineRangeBreak; line += increment) {
		...
		int success = search.Execute(doc->BufferPointer(), startOfLine, endOfLine);
		...
		if (success) {
			...
			if (increment == -1) {
				// Check for the last full match on this line.
				// Continue from the END of the previous full match,
				// not from its start + 1, otherwise reverse search can
				// re-enter the same greedy match and return only its suffix.
				int nextPos = search.eopat[0];

				while (success) {
					if (nextPos <= pos)
						nextPos = doc->MovePositionOutsideChar(pos + 1, 1, false);
					else
						nextPos = doc->MovePositionOutsideChar(nextPos, 1, false);

					if (nextPos > endOfLine)
						break;

					success = search.Execute(doc->BufferPointer(), nextPos, endOfLine);
					if (success) {
						if (search.eopat[0] <= minPos) {
							pos = search.bopat[0];
							lenRet = search.eopat[0] - search.bopat[0];
							nextPos = search.eopat[0];
						} else {
							break;
						}
					}
...
const char *BuiltinRegex::SubstituteByPosition(Document *doc, const char *text, int *length) {
	delete []substituted;
	substituted = 0;
	if (!search.GrabMatches(doc->BufferPointer()))
		return 0;
	unsigned int lenResult = 0;
	for (int i = 0; i < *length; i++) {
		if ((text[i] == '\\') || (text[i] == '$'))
