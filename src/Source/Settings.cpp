...
const wchar_t DISABLE_WORDS_STATS[]		= L"DisableWordsStatistics";
...
void CSettings::InitHotkeyGroups()
{
	...
	// Help
	CHotkey FileHelp(L"Help", IDS_HOTKEY_HELP, NULL, ID_HELP, VK_F1);
    file_hotkeys_group.m_hotkeys.push_back(FileHelp);
...
int CSettings::GetProperties(std::vector<CString>& properties)
{
	...
	properties.push_back(DISABLE_WORDS_STATS);
...
bool CSettings::GetPropertyValue(const CString& sProperty, CProperty& property)
{
	...
	else if(sProperty == DISABLE_WORDS_STATS)
	{
		property = GetStringedProperty(&m_disable_words_stats, KEY_BOOL);
		return true;
	}
...
bool CSettings::SetPropertyValue(const CString& sProperty, CProperty& sValue)
{
	...
	{
		m_disable_words_stats = StrToBool(sValue);
		return true;
	}
	else if(sProperty == WORDS_DLG_POSITION)
	{
...
CString CSettings::GetDefaultScriptsFolder()
{
	...
	::GetModuleFileName(_Module.GetModuleInstance(), filepath, MAX_PATH);
...
bool CSettings::GetDisableWordsStats() const
{
	return m_disable_words_stats;
}
...
void CSettings::SetDisableWordsStats(bool value, bool apply)
{
	m_disable_words_stats = value;
	if(apply)
		Save();
}
...
void CSettings::SetDefaults()
{
	...
	m_disable_words_stats	= false;
	// added by SeNS
	m_usespell_check		= true;
	m_highlght_check		= true;
	m_custom_dict           = L"custom.dic";
	m_custom_dict_codepage	= 65001;
	m_nbsp_char				= L"\u25A1";
	m_keyb_layout			= 0;
	m_change_kbd_layout_check = true;
